...
func TestCreateDefaultChain(t *testing.T) {
	...
	t.Run("3 logger, 2 of them writes to console, second only if it founds"+
		" the world 'hello', third writes to some variable if second found 'hello", func(t *testing.T) {
	...
	t.Run("2 loggers, second uses the closure implementation", func(t *testing.T) {
		...
		if *myWriter.receivedMessage != "Hello closure logger" {
			t.Fatalf("Expected message wasn't received in myWriter")
		}
	})
}
